...
| node.disableCleanup                 | (optional) Disables the deletion of the `/opt/CrowdStrike` directory on the nodes after the Sensor DaemonSet is deleted.                                                                                                 |
| node.cleanupOnly                    | (optional) Cleanup the `/opt/CrowdStrike` directory on all nodes by running the cleanup DaemonSet Only. This will skip the Sensor DaemonSet creation/deletion.                                                           |
