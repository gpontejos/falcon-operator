...
func (r *FalconImageAnalyzerReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	tlsSecretUpdated, err := r.reconcileIARTLSSecret(ctx, req, log, falconImageAnalyzer)
	if err != nil {
		return ctrl.Result{}, err
	}
	...
	if configUpdated || tlsSecretUpdated {
...
func (r *FalconImageAnalyzerReconciler) reconcileIARAgentService(ctx context.Context, req ctrl.Request, log logr.Logger, falconImageAnalyzer *falconv1alpha1.FalconImageAnalyzer) error {
	service := assets.IARAgentService(falconImageAnalyzer)
...
func (r *FalconImageAnalyzerReconciler) reconcileIARTLSSecret(ctx context.Context, req ctrl.Request, log logr.Logger, falconImageAnalyzer *falconv1alpha1.FalconImageAnalyzer) (bool, error) {
	updated := false
	isExpired := false
	notFound := false
	...
	if apierrors.IsNotFound(err) {
		...
		return false, err
	} else if falconImageAnalyzer.Spec.ImageAnalyzerConfig.IARAgentService.AutoCertificateUpdate {
		isExpired, err = tls.IsCertExpired(existingTLSSecret.Data["tls.crt"])
		if err != nil {
			return false, err
		}
	}
	...
	if notFound || isExpired {
		validity := 3650
		if falconImageAnalyzer.Spec.ImageAnalyzerConfig.IARAgentService.CertExpiration != 0 {
			validity = int(falconImageAnalyzer.Spec.ImageAnalyzerConfig.IARAgentService.CertExpiration)
		}
		...
		if notFound {
			...
			if isExpired {
				err = k8sutils.Update(r.Client, ctx, req, log, falconImageAnalyzer, &falconImageAnalyzer.Status, iarTLSSecret)
			}
			if err != nil {
				return false, err
			}
